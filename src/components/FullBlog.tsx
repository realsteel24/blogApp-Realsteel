...
export async function removeBlog(id: string, type: "blog" | "comment") {
  try {
    const response = await fetch(
      `${BACKEND_URL}/api/v1/blog/${type === "comment" ? "comment/" : ""}${id}`,
      {
        method: "DELETE",
        headers: {
    ...
    throw new Error(`An error occurred while deleting the ${type}.`);
  }
}
